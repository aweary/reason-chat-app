...
let authenticate username password => {
  ...
  Js.Promise.( 
    fetchWithInit (apiHost ^ "auth") (RequestInit.make method_::Post body::(BodyInit.make body) headers::headers ())
    |> then_ Response.text
...
let registerUser username password onSuccess onFailure => {
  ...
  let headers = HeadersInit.make {"Content-Type": "application/json"};
  Js.Promise.(
    fetchWithInit (apiHost ^ "signup") (RequestInit.make method_::Post body::(BodyInit.make body) headers::headers ())
    |> then_ (fun response => {
